...
Список тегов, подлежащих проверке на соотвествие справочникам ФНСИ, передается в сервис текстовым ASCII файлов. Теги записыватся построчно. Тег должен содержать следующие атрибуты, значение которых подлежит контролю - code, 1.2.643.5.1.13.13.99.2.48, codeSystemVersion, codeSystemName, displayName. Если есть namespace, то тег указывается с namespace.
