...
-- LEVEL - отключение проверок некоторых аттрибутов. Допустимые значения - ERROR, SKIP_displayName, SKIP_codeSystemName. Значение по умолчанию - ERROR. При значении SKIP_displayName проверка справочника на ошибку INVALID_ELEMENT_VALUE_NAME не выполняется (несоответствие displayName значению поля VALUE справочника). При значении SKIP_codeSystemName не выполняется проверка атрибута codeSystemName.
...
1.2.643.5.1.13.13.99.2.713 - {"LEVEL":"SKIP_codeSystemName", "VALUE":"NAME_FORM"}
```

#### Список тегов, подлежащих проверке на соотвествие справочникам ФНСИ
Список тегов передается в сервис текстовым ASCII файлов. Теги записыватся построчно. Тег должен содержать следующие атрибуты, значение которых подлежит контролю - code, 1.2.643.5.1.13.13.99.2.48, codeSystemVersion, codeSystemName, displayName. Если есть namespace, то тег указывается с namespace.
Пример содежимого:
```
code
