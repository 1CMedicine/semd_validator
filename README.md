# Проект переехал на https://gitverse.ru/1CMedicine/semd_validator

-------------------------------------------
# SEMD validator - сервис проверки СЭМД для 1С:Медицина

Проверяется структурированный электронный медицинский документ (СЭМД) на соответствие xsd схеме, схематрону и справочникам ФНСИ. xsd схему и схематрон публикуются на портале ЕГИСЗ Минздрава РФ. В проекте публикуются схемы и схематроны для СЭМД, которые поддерживаются в 1С:Медицина.
Сервис реализован как сервлет для веб сервера Jetty версии 9.4+. 

## Описание веб-ссылок

### /send_sch.html - загрузка zip-архива со схемой и схематроном, загрузка списка OID справочников ФНСИ, с командами проверки, списка тегов для проверки
...
#### Список OID справочников ФНСИ
Список OID справочников ФНСИ, с командами проверки, передается в сервис текстовым файлом в кодировке UTF-8. OID записыватся построчно, через символ "-" записывается команда проверки. Допустимые команды:
...
-- LEVEL - отключение проверок аттрибута. Допустимые значения - ERROR, SKIP_displayName. Значение по умолчанию - ERROR. При значении SKIP_displayName проверка справочника на ошибку INVALID_ELEMENT_VALUE_NAME не выполняется (несоответствие displayName значению поля VALUE справочника).

-- FULLNAME - задает название справочника. Используется при проверке атрибута codeSystemName.
...
1.2.643.5.1.13.13.99.2.23 - {"LEVEL":"SKIP_displayName"}
1.2.643.5.1.13.13.99.2.713 - {"FULLNAME":"Сопоставление кодов льгот с льготными категориями граждан", "VALUE":"NAME_FORM"}
```

#### Список тегов, подлежащих проверке на соотвествие справочникам ФНСИ
Список тегов передается в сервис текстовым файлом в кодировке UTF-8. Теги записыватся построчно. Тег должен содержать следующие атрибуты, значение которых подлежит контролю - code, 1.2.643.5.1.13.13.99.2.48, codeSystemVersion, codeSystemName, displayName. Если есть namespace, то тег указывается с namespace.
Пример содежимого:
```
code
